...
We work under the following minimal assumptions: ($i$) the marker order is fixed; ($ii$) meioses are independent across offspring; ($iii$) a single female recombination map is shared across dams; ($iv$) genotyping errors are independent across loci and individuals; ($v$) allele transmission follows Mendelian segregation without distortion; ($vi$) the paternal contribution enters the offspring likelihood only through a single per-marker \emph{sire gametic allele frequency} $q_k=\Pr(\text{the paternal gamete transmits allele }A)$, which is integrated out at each marker (detailed later); and ($vii$) the paternal gametic alleles are integrated \emph{independently across markers}, i.e.\ paternal linkage disequilibrium is not modeled.
...
$\boldsymbol\pi_k=(\pi_{2,k},\pi_{1,k},\pi_{0,k})$ be a working sire genotype distribution at marker
$k$ for $\{\mathrm{AA},\mathrm{Aa},\mathrm{aa}\}$ (nonnegative entries summing to one) used only to
carry out this integration; as shown below, the emission---and hence everything the data
identify---depends on $\boldsymbol\pi_k$ only through the single gametic frequency $q_k$, whose
estimation is described later. The emission probability becomes the
...
\paragraph{The identifiable paternal quantity is the gametic allele frequency.}
The sire-averaged emission \eqref{eq:emis-average} depends on the sire genotype vector
$\boldsymbol\pi_k$ only through the \emph{sire gametic allele frequency}
\[
...
\]
with $p_g$ the sire $A$-gamete probability defined above. Equivalently, before genotyping error the
emission is, over offspring genotypes $(\mathrm{AA},\mathrm{Aa},\mathrm{aa})$,
\[
b_k^{(i)}(j)=
\begin{cases}
...
\end{cases}
\]
Consequently the offspring likelihood identifies \emph{only} $q_k$, not the three sire genotype
...
likelihood. We therefore take $q_k$ as the single paternal parameter at each marker (and dam), and
treat $\boldsymbol\pi_k$ only as a redundant working representation; we do not report or interpret
sire genotype frequencies, sire genotype distributions, or Hardy--Weinberg proportions. Under
assumption ($vii$), $q_k$ is integrated independently across markers.
...
Because the emission depends on the sire only through $q_k$ (previous paragraph), we estimate a
single per-marker (and per-dam) gametic frequency $q_k$, updated within the same
Expectation--Maximization (EM) loop \citep{Dempster1977} that estimates $\mathbf r$; full E- and
M-step formulas appear in Section~\ref{sec:par_cont}. The historical Hardy--Weinberg
...
$\lambda=\alpha_k+\beta_k$, a prior mean $q^{(0)}_k$ corresponds to
$\alpha_k=\lambda q^{(0)}_k$ and $\beta_k=\lambda(1-q^{(0)}_k)$. The software default
$\lambda=20$ (i.e.\ $\alpha_k=\beta_k=10$ at prior mean $0.5$) is retained for backward
compatibility only and is \emph{not} a statistically validated recommendation; it can over-shrink
$q_k$ at extreme sire allele frequencies.
...
paternal gametic frequencies $\{q_k\}$. Each EM iteration proceeds as follows: (i) build
$b_k^{(i)}(j)$ from the current $\{q_k\}$; (ii) run forward--backward to obtain $\gamma$ and $\xi$;
(iii) update $\mathbf r$ from $\xi$; and (iv) update each $q_k$ from the $\gamma$-weighted expected
paternal $A$-/$a$-gamete counts using the penalized update of Section~\ref{sec:par_cont}. All
updates share a single Baum--Welch loop; full formulas are provided in Supplementary
Section~\ref{sec:par_cont}.
...
\paragraph{Paternal linkage disequilibrium (limitation).} The paternal contribution is integrated one marker at a time: the sire gametic alleles are treated as independent across markers (assumption $vii$), so paternal linkage disequilibrium (LD) is not modeled. When paternal LD is present, the offspring two-locus genotype distribution departs from the independent-gamete form assumed here, which can bias the sire allele-frequency estimates and, through them, confound maternal phase or recombination estimates---most directly in the pairwise phase stage (Section~\ref{sec:tpt}), whose likelihood assumes independent paternal alleles at the two markers. The present model therefore does not claim to resolve unknown-paternity structure completely; accounting for paternal LD is left to future work.
...
The paternal contribution at each marker \(k\) is summarized by a single gametic frequency
\(q_k=\Pr(\text{the paternal gamete transmits }A)\), integrated independently across markers
(assumption $vii$). Its update is embedded in the same Baum--Welch loop used to estimate
\(\mathbf r\). Let
\[
...
the emission is \(b_k^{(i)}(j)=q_k\,e^{A}_{ik}(j)+(1-q_k)\,e^{a}_{ik}(j)\). Each outer iteration
(i) builds \(b_k^{(i)}(j)\) from the current \(q_k\); (ii) runs forward--backward to obtain
\(\gamma\) and \(\xi\); (iii) updates \(\mathbf r\) from \(\xi\); and (iv) updates \(q_k\) as follows.

\paragraph{E-step: expected paternal gamete counts.}
For offspring \(i\) at marker \(k\), the posterior probability that the paternal gamete transmitted
\(A\), marginalized over the maternal-state posterior \(\gamma_k^{(i)}(j)\) with the emission
normalized \emph{per maternal state}, is
\[
\rho_{A,k}^{(i)}=\sum_{j\in\{1,2\}}\gamma_k^{(i)}(j)\,
\frac{q_k\,e^{A}_{ik}(j)}{b_k^{(i)}(j)},
\qquad
\rho_{a,k}^{(i)}=\sum_{j\in\{1,2\}}\gamma_k^{(i)}(j)\,
\frac{(1-q_k)\,e^{a}_{ik}(j)}{b_k^{(i)}(j)},
\]
which sum to \(1\) at observed loci. A maternal state \(j\) contributes only when both
\(\gamma_k^{(i)}(j)>0\) and \(b_k^{(i)}(j)>0\), so an impossible state at zero genotyping error is
skipped rather than dividing by zero. Missing calls (\(y_{ik}=\varnothing\)) contribute no gamete
count. Summing over offspring gives the expected paternal \(A\)- and \(a\)-gamete counts
\[
N_{A,k}=\sum_{i=1}^{n}\rho_{A,k}^{(i)},\qquad
N_{a,k}=\sum_{i=1}^{n}\rho_{a,k}^{(i)}.
\]

\paragraph{M-step: penalized (MAP) update.}
With a per-marker pseudocount prior \((\alpha_k,\beta_k)\),
\[
\hat q_k=\frac{N_{A,k}+\alpha_k}{N_{A,k}+N_{a,k}+\alpha_k+\beta_k}.
\]
This maximizes the penalized objective
\(\log\mathcal{L}+\sum_k\bigl[\alpha_k\log q_k+\beta_k\log(1-q_k)\bigr]\); equivalently it is the
posterior mode under a \(\mathrm{Beta}(\alpha_k+1,\beta_k+1)\) prior, and \(\alpha_k=\beta_k=0\)
gives the unregularized fit. Writing the common total pseudocount \(\lambda=\alpha_k+\beta_k\) and a
prior mean \(q^{(0)}_k\), one sets \(\alpha_k=\lambda q^{(0)}_k\) and
\(\beta_k=\lambda(1-q^{(0)}_k)\); the software default \(\lambda=20\) (\(\alpha_k=\beta_k=10\) at
prior mean \(0.5\)) is retained for backward compatibility only and is not a statistically
validated recommendation.

\paragraph{Practical notes.}
(i) Initialization: \(q^{(0)}_k=0.5\) when no prior is supplied.
(ii) Missing data: if all calls at marker \(k\) are missing, \(\hat q_k\) reverts to the prior mean.
(iii) Numerical stability: clip \(\hat q_k\) to \([10^{-9},\,1-10^{-9}]\).
(iv) The Hardy--Weinberg genotype form \(\boldsymbol{\pi}_k=[\,p_k^2,\,2p_k(1-p_k),\,(1-p_k)^2\,]\)
with \(p_k=q_k\) yields identical emissions and is only a reparameterization of this one-parameter
model, not a separately identifiable model.
...
Let $p_k$ be the frequency of allele $A$ transmitted by the sire at marker $k$; under the
independent-marker assumption ($vii$) this is exactly the sire gametic frequency $q_k$ of the
multipoint model, so the two-point stage is parameterized by the same identifiable quantity.

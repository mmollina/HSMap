...
struct EStepWorker : public Worker {
  ...
  // Per-marker emission-error rates. In the global model every entry equals
  // the scalar epsilon, so all arithmetic is bit-identical to the historical
  // scalar path; the marker_specific model updates the entries between
  // iterations.
  RVector<double>  eps;      // length T
  bool             track_err; // accumulate expected error-channel counts?
  int              T;
  ...
  std::vector<double> Eerr;          // length T, expected error-channel counts (marker_specific)
  ...
  EStepWorker(IntegerMatrix G_,
              ...
              NumericVector eps_,
              bool track_err_ = false)
    : G(G_), M(M_), phase(phase_), r(r_),
      pi_emis(pi_emis_), Pi(Pi_), paternal_mode(paternal_mode_),
      eps(eps_), track_err(track_err_),
      T(G_.ncol()),
  ...
  EStepWorker(EStepWorker& rhs, Split)
    : G(rhs.G), M(rhs.M), phase(rhs.phase), r(rhs.r),
      pi_emis(rhs.pi_emis), Pi(rhs.Pi), paternal_mode(rhs.paternal_mode),
      eps(rhs.eps), track_err(rhs.track_err),
      T(rhs.T),
      same(T-1, 0.0), diff(T-1, 0.0),
      Ng0(T, 0.0), Ng1(T, 0.0), Ng2(T, 0.0),
      N_A(T, 0.0), N_a(T, 0.0),
  ...
  inline double fb_child_and_accumulate(std::size_t i) {
    ...
    for (int t=0; t<T; ++t) {
      const int y = G(i,t);
      double e[2];
      emission_vec_wrapped(y, pi_emis, t, M[t], eps[t], e);
      E[2*t+0] = e[0];
      E[2*t+1] = e[1];
    }
    ...
    // Error-channel responsibilities (marker_specific epsilon only).
    //
    // The emission is the two-component mixture
    //   P(y | h) = (1 - eps_t) * p0(y|h) + (eps_t/2) * (1 - p0(y|h)),
    // where p0 is the CLEAN emission (the same paternal mixture evaluated at
    // eps = 0). The posterior that call (i,t) came through the error channel,
    // given maternal state h, is therefore
    //   P(err | y, h) = (eps_t/2) * (1 - p0(y|h)) / P(y | h),
    // and the per-call responsibility integrates over h with the state
    // posterior gamma(h) = alpha_t(h) beta_t(h) / sum_h'. Missing calls
    // contribute nothing. This is the exact E-step for the latent error
    // indicator; the MAP M-step is eps_t = (E_t + tau eps0) / (N_t + tau).
    if (track_err) {
      for (int t=0; t<T; ++t) {
        const int y = G(i,t);
        ...
        if (z <= 0.0) continue;
        double p0[2];
        emission_vec_wrapped(y, pi_emis, t, M[t], 0.0, p0);   // clean emission
        const double half_eps = 0.5 * eps[t];
        double acc = 0.0;
        if (E[2*t+0] > 0.0)
          acc += (gg0 / z) * half_eps * (1.0 - p0[0]) / E[2*t+0];
        if (E[2*t+1] > 0.0)
          acc += (gg1 / z) * half_eps * (1.0 - p0[1]) / E[2*t+1];
        Eerr[t] += acc;
      }
    }
    ...
    if (paternal_mode == "HWE") {
      for (int t=0; t<T; ++t) {
        ...
        if (ga0 > 0.0) {
          const double eA = py_hgM_obs(y,0,1,Mt,eps[t]); // P(y | maternal 0, gamete A)
          const double ea = py_hgM_obs(y,0,3,Mt,eps[t]); // P(y | maternal 0, gamete a)
          const double b0 = q*eA + (1.0-q)*ea;
          if (b0 > 0.0) { nA += ga0 * q * eA / b0; na += ga0 * (1.0-q) * ea / b0; }
        }
        if (ga1 > 0.0) {
          const double eA = py_hgM_obs(y,1,1,Mt,eps[t]);
          const double ea = py_hgM_obs(y,1,3,Mt,eps[t]);
      ...
      for (int t=0; t<T; ++t) {
        ...
        if (ga0 > 0.0) {
          const double pAA = py_hgM_obs(y,0,1,Mt,eps[t]);
          const double pAa = py_hgM_obs(y,0,2,Mt,eps[t]);
          const double paa = py_hgM_obs(y,0,3,Mt,eps[t]);
        ...
        if (ga1 > 0.0) {
          const double pAA = py_hgM_obs(y,1,1,Mt,eps[t]);
          const double pAa = py_hgM_obs(y,1,2,Mt,eps[t]);
          const double paa = py_hgM_obs(y,1,3,Mt,eps[t]);
...
Rcpp::List hmm_hs_cpp_parallel(Rcpp::IntegerMatrix G,           // n x T
                               ...
                               std::string epsilon_model = "global", // "global" | "marker_specific"
                               double epsilon_tau = 100.0) // shrinkage strength toward epsilon_0
{
  const int n = G.nrow();
  const int T = G.ncol();
  ...
  if (!(epsilon_model == "global" || epsilon_model == "marker_specific"))
    stop("epsilon_model must be 'global' or 'marker_specific'");
  const bool ms = (epsilon_model == "marker_specific");
  if (ms && paternal_mode != "HWE")
    stop("epsilon_model = 'marker_specific' requires the gametic/HWE paternal model");
  if (ms && !(epsilon_tau > 0.0))
    stop("epsilon_tau must be > 0");
  // Marker-specific emission-error rates eps_k, shrunk toward the baseline
  // eps0 = epsilon under eps_k ~ Beta(1 + tau*eps0, 1 + tau*(1-eps0)), whose
  // MAP update given expected error counts is eps_k = (E_k + tau*eps0)/(N_k + tau).
  // In global mode every entry stays equal to the scalar epsilon and every
  // computation is bit-identical to the historical path.
  const double eps0 = std::min(0.49, std::max(1e-8, epsilon));
  NumericVector eps_vec(T, epsilon);
  std::vector<int> Nobs(T, 0);      // observed calls per marker (excludes NA)
  for (int t=0; t<T; ++t) {
    int c = 0;
    for (int i=0; i<n; ++i) if (G(i,t) != NA_INTEGER) ++c;
    Nobs[t] = c;
  }
  ...
  // plus the Beta prior on eps_k when epsilon_model = "marker_specific".
  const bool has_pen = (paternal_mode == "HWE" && lambda > 0.0);
  const double eps_a = epsilon_tau * eps0;            // Beta(1+a, 1+b) exponents
  const double eps_b = epsilon_tau * (1.0 - eps0);
  auto eps_penalty = [&](void)->double {
    if (!ms) return 0.0;
    double p = 0.0;
    for (int t=0; t<T; ++t)
      p += eps_a * std::log(eps_vec[t]) + eps_b * std::log(1.0 - eps_vec[t]);
    return p;
  };
  ...
  for (it=1; it<=maxit; ++it) {
    ...
    // parallel E-step
    EStepWorker worker(G, M, phase_vec, r, pi_emis, Pi, paternal_mode,
                       eps_vec, ms);
    ...
    // MAP M-step for marker-specific epsilon (committed after the objective is
    // evaluated at the CURRENT eps, mirroring the r update pattern)
    NumericVector eps_new(T);
    double deps = 0.0;
    if (ms) {
      for (int t=0; t<T; ++t) {
        double e = (worker.Eerr[t] + epsilon_tau * eps0) /
                   std::max(static_cast<double>(Nobs[t]) + epsilon_tau, 1e-12);
        e = std::min(0.49, std::max(1e-6, e));
        eps_new[t] = e;
        deps = std::max(deps, std::fabs(e - eps_vec[t]));
      }
    }
    ...
    const double pen_eps_here = eps_penalty();   // eps_vec not yet updated
    const double active_obj = total_ll + pen_here + pen_eps_here;
    ll_trace.push_back(total_ll);
    penobj_trace.push_back((has_pen || ms) ? (total_ll + pen_here + pen_eps_here)
                                           : NA_REAL);
    obj_trace.push_back(active_obj);
    (void) dpi;   // non-identifiable paternal decomposition change is NOT a gate

    // identifiable parameter changes: r, q and (marker_specific) eps
    double dr = 0.0;
    ...
    // Convergence on the ACTIVE objective plus stable identifiable r, q, eps.
    ...
    if (ms) for (int t=0; t<T; ++t) eps_vec[t] = eps_new[t];
  ...
  double final_ll;
  NumericVector Eerr_final(T);
  {
    EStepWorker fw(G, M, phase_vec, r, pi_emis, Pi, paternal_mode, eps_vec, ms);
    parallelReduce(0, static_cast<std::size_t>(n), fw);
    final_ll = fw.ll_sum;
    if (ms) for (int t=0; t<T; ++t) Eerr_final[t] = fw.Eerr[t];
  }
  const double final_pen = q_penalty() + eps_penalty();
  const double final_penobj = final_ll + final_pen;
  const double final_obj = (has_pen || ms) ? final_penobj : final_ll;

  // Append the FINAL values so each trace ends exactly at the returned quantity.
  ll_trace.push_back(final_ll);
  penobj_trace.push_back((has_pen || ms) ? final_penobj : NA_REAL);
  ...
  List out = List::create(
    ...
    _["epsilon"]            = epsilon,
    _["epsilon_model"]      = epsilon_model,
    _["paternal_mode"]      = paternal_mode
  );
  if (ms) {
    out["epsilon_k"]   = eps_vec;
    out["epsilon_0"]   = eps0;
    out["epsilon_tau"] = epsilon_tau;
    out["E_k"]         = Eerr_final;
    out["N_k"]         = Rcpp::wrap(Nobs);
  }
  if (paternal_mode == "two_locus") {
    out["Pi_interval"] = Pi;
                            ...
                            Rcpp::Nullable<Rcpp::List> Pi_prior_list_in = R_NilValue) // list of 10 x (T-1)
{
  ...
  // The joint engine supports the GLOBAL epsilon model only; the worker takes
  // a per-marker vector, so pass one filled with the scalar (bit-identical).
  NumericVector eps_vec_j(T, epsilon);
  ...
  for (it=1; it<=maxit; ++it) {
    ...
    for (int d=0; d<D; ++d) {
      ...
      EStepWorker worker(G[d], M[d], Ph[d], r, pi_emis, Pi[d], paternal_mode,
                         eps_vec_j, false);
  ...
  for (int d=0; d<D; ++d) {
    ...
    EStepWorker fw(G[d], M[d], Ph[d], r, pe, Pi[d], paternal_mode,
                   eps_vec_j, false);

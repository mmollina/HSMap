...
.pf_components <- function(Sup) {
  # Iterative DFS. Two details keep this O(n^2) on dense phase graphs, which is
  # where linkage groups live when `min_phase_lod` admits most pairs:
  #   (i)  vertices are labelled when PUSHED, not when popped, so each vertex
  #        enters the stack exactly once and the stack never exceeds n. Marking
  #        on pop lets a vertex be pushed once per visited neighbour, growing
  #        the stack to O(n^2);
  #   (ii) the stack is preallocated with a top pointer. `stack[-length(stack)]`
  #        copies the whole stack on every pop, which with (i) made the cost
  #        grow like n^4 (a 600-marker clique took ~65 s; this takes ~0.02 s).
  # Component labelling is unchanged: the outer sweep still opens components in
  # ascending vertex order, so the returned ids are identical to before.
  n <- nrow(Sup); comp <- integer(n); cid <- 0L
  stack <- integer(n)
  for (s in seq_len(n)) {
    if (comp[s] != 0L) next
    cid <- cid + 1L
    top <- 1L; stack[1L] <- s; comp[s] <- cid
    while (top > 0L) {
      v <- stack[top]; top <- top - 1L
      nb <- which(Sup[v, ] & comp == 0L)
      if (length(nb)) {
        comp[nb] <- cid                                  # mark on push
        stack[(top + 1L):(top + length(nb))] <- nb
        top <- top + length(nb)
      }
    }
  }

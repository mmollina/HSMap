...
#' @param thresh.LOD.ph Numeric. \strong{Despite its name, this does NOT
#'   filter by phase LOD.} The mask screens \code{lod_r} (linkage LOD) only,
#'   using the single effective cutoff
#'   \code{max(thresh.LOD.ph, thresh.LOD.rf)} -- so setting
#'   \code{thresh.LOD.ph = 10} silently raises the \emph{linkage} threshold to
#'   10. The argument is retained for backward compatibility with existing
#'   scripts; use \code{thresh.LOD.rf} to express the linkage cutoff you mean.
#'   A phase-LOD screen may be added under a correctly named argument in a
#'   future release. Default \code{5}.

...
cpp_pairwise_rf <- function(G_list,
                            ...
                            optimizer = c("auto", "grid", "newton")) {
  # "auto" (default): single-family analyses use the exact concave
  # phase-decomposed Newton solve; multi-family analyses fall back to the grid.
  # "grid" forces the reference grid implementation (regression/comparison);
  # "newton" is single-family only and errors otherwise.
...
#'     \eqn{\hat r = 0.5} is permitted:
#'     \deqn{ \mathrm{Obj}(r) = \sum_{g=1}^{G} \max\{\ell_g(\mathrm{Coupling}; r), \ell_g(\mathrm{Repulsion}; r)\} }
#'     For a \strong{single family} (\eqn{G = 1}), each phase-specific
#'     log-likelihood is concave in \eqn{r} (every cell probability is affine in
#'     \eqn{r}), and \eqn{\max_r \max(\ell_C, \ell_R) =
#'     \max(\max_r \ell_C, \max_r \ell_R)}, so each phase is maximized exactly by
#'     a safeguarded derivative-based (Newton/bisection) solve and the larger
#'     maximum is kept. For \strong{several families} the objective is a sum of
#'     per-dam maxima, the decomposition no longer holds and the objective may be
#'     multimodal, so it is maximized by a coarse grid over
...
pairwise_rf <- function(
  ...
  # Optimizer selection is automatic: a single family uses the exact concave
  # phase-decomposed Newton solve, several families use the grid+refinement
  # search. The internal option below exists for regression/comparison testing
  # only (e.g. options(HSMap.two_point_optimizer = "grid") forces the reference
  # grid path); it is not part of the public API.
  optimizer <- getOption("HSMap.two_point_optimizer", "auto")

...
#' @param epsilon_model Emission-error model. \code{"global"} (default; the
#'   historical model, numerically unchanged): one shared error rate
#'   \code{epsilon}. \code{"marker_specific"}: each marker gets its own rate
#'   \eqn{\epsilon_k}, estimated inside the EM with shrinkage toward the
#'   baseline \eqn{\epsilon_0 = } \code{epsilon} under the prior
#'   \eqn{\epsilon_k \sim Beta(1 + 	au\epsilon_0, 1 + 	au(1-\epsilon_0))},
#'   giving the MAP update \eqn{\epsilon_k = (E_k + 	au\epsilon_0)/(N_k +
#'   	au)} where \eqn{E_k} is the posterior expected error-channel count and
#'   \eqn{N_k} the observed calls at marker \eqn{k}. The emission geometry is
#'   unchanged (symmetric; off-diagonal \eqn{\epsilon_k/2}). The active
#'   objective adds the Beta prior term. Estimates are returned as
#'   \code{fit$epsilon_k} (with \code{fit$E_k}, \code{fit$N_k},
#'   \code{fit$epsilon_0}, \code{fit$epsilon_tau}). Interpret
#'   \eqn{\epsilon_k} as a marker-specific emission-error/incompatibility
#'   rate, not as proven genotyping error. Single-dam fits only; requires the
#'   gametic paternal model.
#' @param epsilon_tau Shrinkage strength \eqn{	au} (prior pseudo-observations
#'   pulling \eqn{\epsilon_k} toward \eqn{\epsilon_0}). Default \code{100}.
...
hmm_map <- function(
    ...
    threads = NULL,
    epsilon = 0.01,
    epsilon_model = c("global", "marker_specific"),
    epsilon_tau = 100,
    tol = 1e-6,
  ...
  epsilon_model <- match.arg(epsilon_model)
  if (identical(epsilon_model, "marker_specific")) {
    if (!is.numeric(epsilon_tau) || length(epsilon_tau) != 1L || epsilon_tau <= 0)
      stop("`epsilon_tau` must be a single positive number.", call. = FALSE)
    if (!identical(.hsmap_paternal_engine(paternal_mode), "HWE"))
      stop("epsilon_model = \"marker_specific\" requires the gametic/HWE ",
           "paternal model.", call. = FALSE)
  }
  ...
  run_one <- function(ph) {
    ...
    if (!is.null(fit$epsilon_k)) {
      fit$epsilon_k <- stats::setNames(as.numeric(fit$epsilon_k), ord)
      fit$E_k <- stats::setNames(as.numeric(fit$E_k), ord)
      fit$N_k <- stats::setNames(as.integer(fit$N_k), ord)
    }
  ...
  if (identical(method, "joint")) {
    if (identical(epsilon_model, "marker_specific"))
      stop("epsilon_model = \"marker_specific\" is implemented for single-dam ",
           "fits only.", call. = FALSE)
